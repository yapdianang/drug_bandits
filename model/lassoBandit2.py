...
class LASSOBandit(object):
    ...
    def __init__(self, q, h, lambda1, lambda2, nb_feature_dims, K=3):
        ...
        for i in range(1, self.K+1):
            self.forced_params[i] = Lasso(fit_intercept=True, alpha=(self.lambda1/2), max_iter=10000)
            # self.forced_params[i].coeff_ = np.zeros((self.d,))
            self.all_params[i]    = Lasso(fit_intercept=True, alpha=(self.lambda2/2), max_iter=10000)
